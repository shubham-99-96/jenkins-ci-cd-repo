...
//https://employer.tankhwaa.com/view_my_resume?job=true&employee_id=lZk=
...
if(check_employee()) {
    ...
    } if(check_params($_GET,["hire","employee_id"])) {
        $employee_id = decrypt($_GET["employee_id"],"cand74");
        if(!$Jobs->check_hired_candidates($_SESSION["employer_id"],$employee_id)) {
            $profile_data = $Employees->get_full_employee($employee_id);
        } else {
            set_alert("danger","Access Denied ..... code=2");
            header("location:index");
            die();
        }
    } else {
...
if(isset($_POST["Download_Resume"])) {
    if(count($profile_data) == 1) {
        $filePath = dirname(__DIR__) . "/Admin/Public/candidate_resume/" . $profile_data[0]["resume"];
